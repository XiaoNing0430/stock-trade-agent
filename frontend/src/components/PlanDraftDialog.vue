...
const mergedWarnings = computed(() => [...(assist.draft?.warnings ?? []), ...(assist.suggestion?.warnings ?? [])]);
...
<template>
  <div v-if="assist.visible" class="dialog-backdrop" @click.self="assist.close()">
    <section class="dialog-panel plan-draft-panel" role="dialog" aria-modal="true" aria-label="交易计划草案">
      <header class="plan-draft-head">
        <strong>交易计划草案{{ assist.draft ? ` · ${assist.draft.name || assist.draft.code}` : '' }}</strong>
        <span v-if="assist.draft?.fallbackUsed" class="draft-fallback-badge" data-testid="fallback-badge"
          >备用数据源</span
        >
        <button class="text-button" type="button" @click="assist.close()">关闭</button>
      </header>

      <div v-if="assist.loading" class="plan-draft-state">草案生成中…</div>
      <div v-else-if="assist.error" class="plan-draft-state plan-draft-error">
        {{ assist.error }}
      </div>

      <template v-else-if="assist.draft">
        <div v-if="assist.draft.stale" class="draft-stale-banner" data-testid="stale-banner">
        ...
        <dl class="draft-metrics">
          <div>
            <dt>数据日期</dt>
            <dd>{{ assist.draft.referenceDate || '--' }}</dd>
          </div>
          <div>
            <dt>数据来源</dt>
            <dd>{{ assist.draft.provider || '--' }}</dd>
          </div>
          <div>
            <dt>入场价</dt>
            <dd>
            ...
            <p class="field-hint" data-testid="draft-price-scope-note">
              计划价格以原始实时价为准；K 线图为前复权价，请勿直接照抄图表价位。
            </p>
          </div>
        </dl>

        <ul v-if="mergedWarnings.length" class="draft-warnings" data-testid="warnings">
          <li v-for="(warning, index) in mergedWarnings" :key="index" data-testid="warning-item">
            {{ warning }}
          </li>
        </ul>
        ...
        <fieldset class="draft-params">
          ...
          <div class="draft-params-grid">
            <label
              >止损模式
              <select v-model="params.stopMode" data-testid="stop-mode-select">
        ...
        <details class="draft-kelly" data-testid="kelly-ref">
        ...
        <dl class="draft-result">
          <div>
            <dt>止损价</dt>
            <dd>{{ formatNumber(assist.suggestion?.stop ?? null) }}</dd>
          </div>
          <div data-testid="target">
            <dt>目标价</dt>
            <dd>{{ formatNumber(assist.suggestion?.target ?? null) }}</dd>
          </div>
          <div data-testid="shares">
            <dt>建议股数</dt>
            <dd>{{ assist.suggestion ? `${assist.suggestion.suggestedShares} 股` : '--' }}</dd>
          </div>
          <div data-testid="position">
            <dt>仓位占比</dt>
            <dd>{{ assist.suggestion ? `${formatNumber(assist.suggestion.positionPct)}%` : '--' }}</dd>
          </div>
          <div>
            <dt>账户权益</dt>
            <dd>{{ equityLabel }}</dd>
        ...
        <p
          v-if="
            assist.suggestion &&
            !assist.submitting &&
            (assist.suggestion.stop == null || assist.suggestion.suggestedShares === 0)
          "
          class="draft-disable-hint"
          data-testid="confirm-hint"
        >
          {{
            assist.suggestion.stop == null
              ? '暂无有效止损，请调整参数或手动设定止损'
        ...
        <footer class="plan-draft-foot">
          <button class="button button-secondary" type="button" @click="assist.close()">取消</button>
          <button
            ...
            :disabled="
              ...
              !assist.suggestion ||
              assist.suggestion.stop == null ||
              assist.suggestion.suggestedShares === 0
            "
            @click="onConfirm"
          >
