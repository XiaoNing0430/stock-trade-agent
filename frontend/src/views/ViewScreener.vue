<template>
  <section class="view-panel is-active screener-view">
    ...
    <div class="screener-tabs">
      <button
        :class="['screener-tab', { 'is-active': screenerMode === 'featured' }]"
        type="button"
        @click="switchScreenerMode('featured')"
      >
        精选 50
      </button>
      <button
        :class="['screener-tab', { 'is-active': screenerMode === 'all' }]"
        type="button"
        @click="switchScreenerMode('all')"
      >
        全市场
      </button>
      <button
        :class="['screener-tab', { 'is-active': screenerMode === 'strategy' }]"
        type="button"
        @click="openStrategyTab()"
      >
        策略
      </button>
    </div>
    ...
    <div v-if="screenerMode === 'strategy'" class="screener-layout">
      <section class="filter-panel surface">
        ...
        <div class="field-grid">
          ...
          <div class="field">
            ...
            <div class="screener-tabs strategy-mode-tabs">
              <button
                :class="['screener-tab', { 'is-active': strategyRunMode === 'quick' }]"
                type="button"
                @click="switchStrategyMode('quick')"
              >
                极速
              </button>
              <button
                :class="['screener-tab', { 'is-active': strategyRunMode === 'deep' }]"
                type="button"
                @click="switchStrategyMode('deep')"
              >
                深度
              </button>
            </div>
          ...
          <div class="field">
            <span>立即扫描</span>
            <button class="button" type="button" data-testid="scan-now" :disabled="scanRunning" @click="onScanNow">
        ...
        <p class="heading-note">
          {{ strategyDescription
          }}{{ strategyRunMode === 'deep' ? '（深度模式需逐票拉取历史数据，预计 10–30 秒）' : '' }}
        </p>
      ...
      <div class="results-panel surface">
        <div class="surface-heading">
          <div>
            ...
            <h3>
              命中结果
              <span v-if="strategyReferenceDate" class="heading-note">基准日期 {{ strategyReferenceDate }}</span>
            </h3>
          </div>
          <span v-if="strategyCached" class="scan-time">{{
            strategyStale ? '⚠ 数据可能滞后（缓存降级）' : '缓存命中'
          }}</span>
        </div>
        <div v-if="strategyStale" class="empty-state">
          <i data-lucide="alert-triangle" aria-hidden="true"></i><strong>数据可能滞后</strong
          ><span>数据源暂时不可用，以下为上次缓存结果，请稍后强制刷新。</span>
        </div>
          ...
          <table class="data-table data-table-dense">
            ...
            <tbody>
              ...
              >
                ...
                <td>
                  <span v-for="tag in strategyFactorTags(stock)" :key="tag" class="sort-arrow factor-tag">{{
                    tag
                  }}</span
                  ><span v-if="!strategyFactorTags(stock).length">--</span>
                </td>
                ...
                <td class="text-end">
                  <button
                    class="text-button"
                    type="button"
                    :data-testid="'draft-' + stock.code"
                    @click.stop="openDraft(stock)"
                  >
                    草案
                  </button>
                  <button
                    class="text-button"
                    type="button"
                    :data-testid="'backtest-' + stock.code"
                    @click.stop="openBacktest(stock)"
                  >
                    回测
                  </button>
                  <button
                    class="table-action"
                    type="button"
                    :aria-label="isWatched(stock.code) ? '移出自选' : '加入自选'"
                    :data-tooltip="isWatched(stock.code) ? '移出自选' : '加入自选'"
                    @click.stop="toggleWatch(stock.code)"
                  >
                    <i :data-lucide="isWatched(stock.code) ? 'star-off' : 'star'" aria-hidden="true"></i>
                  </button>
                </td>
              </tr>
            </tbody>
        ...
        <div v-if="strategyLoading" class="empty-state">
          <i data-lucide="loader-circle" aria-hidden="true"></i><strong>正在运行策略</strong
          ><span>{{ strategyRunMode === 'deep' ? '深度模式需逐票计算因子，请稍候。' : '极速模式筛选中…' }}</span>
        </div>
        <div v-else-if="!strategyRows.length" class="empty-state">
          <i data-lucide="search-x" aria-hidden="true"></i><strong>暂无结果</strong
          ><span>点击「运行策略」开始选股。</span>
        </div>
      </div>
    </div>
      ...
      <section class="filter-panel surface">
        ...
        <div class="preset-list">
          <button
            ...
            :key="preset.name"
            class="preset-item"
            :class="{ 'is-active': presetName === preset.name }"
            type="button"
        ...
        <div class="filter-fields">
          ...
          <label class="field"
            ...
            ><select v-model="filters.market">
              ...
              <option value="北交所">北交所</option>
              <option value="沪市ETF">沪市 ETF</option>
              <option value="深市ETF">深市 ETF</option>
              <option value="科创板ETF">科创板 ETF</option>
            </select></label
          >
          <label class="field"
            ><span>搜索</span><input v-model.trim="filters.search" type="search" placeholder="代码 / 名称"
          /></label>
          <label class="field"
            ><span
              >PE ≤ <strong>{{ filters.peMax }}</strong></span
            ><input v-model.number="filters.peMax" type="range" min="5" max="120" step="1"
          /></label>
          <label class="field"
            ><span
              >PB ≤ <strong>{{ filters.pbMax }}</strong></span
            ><input v-model.number="filters.pbMax" type="range" min="0.5" max="20" step="0.5"
          /></label>
          <label class="field"
            ><span
              >量比 ≥ <strong>{{ filters.volumeMin.toFixed(2) }}</strong></span
            ><input v-model.number="filters.volumeMin" type="range" min="0.2" max="8" step="0.1"
          /></label>
          <label class="field"
            ><span
              >涨幅 ≥ <strong>{{ filters.changeMin.toFixed(1) }}%</strong></span
            ><input v-model.number="filters.changeMin" type="range" min="-10" max="10" step="0.1"
          /></label>
        </div>
      ...
      <section class="results-panel surface">
        <div class="surface-heading results-heading">
          <div>
            <span class="section-kicker">RESULTS</span>
            <h3>
              <span>{{ filteredRows.length }}</span> 只股票符合条件
            </h3>
          </div>
              ...
              >
                <td>
                  <div class="stock-cell">
                    ...
                    <div class="stock-cell-copy">
                      <strong>{{ stock.name }}</strong
                      ><span>{{ stock.code }} · {{ stock.exchange }} · {{ stock.board }}</span>
                    </div>
                  </div>
                </td>
                <td class="text-end">{{ formatNullable(stock.price) }}</td>
                ...
                <td>
                  <span :class="['signal-chip', signalClass(stock)]">{{ signalText(stock) }}</span>
                </td>
                <td class="text-end">
                  <button
                    class="table-action"
        ...
        <div v-if="loading && !filteredRows.length" class="empty-state">
          <i data-lucide="loader-circle" aria-hidden="true"></i><strong>正在读取真实行情</strong
          ><span>行情列表返回后会显示筛选结果。</span>
        </div>
        <div v-else-if="!filteredRows.length" class="empty-state">
          <i data-lucide="search-x" aria-hidden="true"></i><strong>没有找到符合条件的股票</strong
          ><span>可以放宽 PE、PB 或涨幅条件再试一次。</span>
        </div>
      </section>
    </div>
    ...
    <div v-else class="all-market-panel surface">
      <div class="surface-heading results-heading">
        <div>
          ...
          <h3>
            <span>{{ screenerAllTotal.toLocaleString() }}</span> 只沪深 A 股
          </h3>
        </div>
        <div class="results-actions">
      ...
      <div class="table-responsive results-table-wrap">
        <table class="data-table data-table-dense">
          <thead>
            <tr>
              <th class="screener-sort" @click="screenerSort('code')">
                代码 <span class="sort-arrow">{{ screenerSortIcon('code') }}</span>
              </th>
              <th>名称</th>
              <th class="text-end screener-sort" @click="screenerSort('price')">
                现价 <span class="sort-arrow">{{ screenerSortIcon('price') }}</span>
              </th>
              <th class="text-end screener-sort" @click="screenerSort('changePct')">
                涨跌幅 <span class="sort-arrow">{{ screenerSortIcon('changePct') }}</span>
              </th>
              <th class="text-end screener-sort" @click="screenerSort('amount')">
                成交额 <span class="sort-arrow">{{ screenerSortIcon('amount') }}</span>
              </th>
              <th class="text-end">换手率</th>
              <th class="text-end">量比</th>
              <th class="text-end">PE</th>
          ...
          <tbody>
            <tr
              v-for="stock in screenerAllRows"
              :key="stock.symbol"
              class="clickable-row"
              @click="selectStock(stock.code)"
            >
              ...
              <td class="text-end">{{ formatNullable(stock.pb, 2) }}</td>
              <td class="text-end">
                {{ stock.totalMarketCap != null ? stock.totalMarketCap.toLocaleString() + ' 亿' : '--' }}
              </td>
              <td class="text-end" :class="trendClass(stock.netMoneyFlow)">
                {{ stock.netMoneyFlow != null ? stock.netMoneyFlow.toLocaleString() + ' 万' : '--' }}
              </td>
              <td class="text-end">
                <button
                  class="table-action"
      ...
      <div v-if="screenerLoading" class="empty-state">
        <i data-lucide="loader-circle" aria-hidden="true"></i><strong>正在读取全市场行情</strong
        ><span>腾讯排名接口返回后会显示结果。</span>
      </div>
      <div v-else-if="!screenerAllRows.length" class="empty-state">
        <i data-lucide="search-x" aria-hidden="true"></i><strong>暂时没有数据</strong
        ><span>全市场排名接口暂时不可用，请稍后重试。</span>
      </div>
      <div class="pagination">
        <button class="button button-secondary" type="button" :disabled="screenerPage <= 1" @click="screenerPageDown">
        ...
        <button
          ...
          type="button"
          :disabled="screenerPage >= Math.ceil(screenerAllTotal / 50)"
          @click="screenerPageUp"
        >
...
async function onScanToggle(checked: boolean): Promise<void> {
  ...
  if (ok) {
    scanState.value = {
      ...(scanState.value ?? {
        strategyId: String(strategyName.value),
        strategyName: '',
        lastRunAt: null,
        lastStatus: null,
        hitCount: 0,
