<template>
  <section class="view-panel is-active">
    ...
    <template v-if="hubTab === 'settings'">
      ...
      <section v-if="settingsTab === 'data'" class="settings-row"><div><strong>实时行情来源</strong><span>实时 A 股报价主数据源</span></div><select v-model="settingsDraft.realtimeSource" aria-label="实时行情来源"><option v-for="s in dataSources.filter((s: any) => s.realtime)" :key="s.id" :value="s.id" :disabled="!s.available">{{ s.name }}<template v-if="!s.available">（{{ s.reason || '不可用' }}）</template></option></select></section>
      <section v-if="settingsTab === 'data'" class="settings-row"><div><strong>历史日线来源</strong><span>用于走势和网格回测</span></div><select v-model="settingsDraft.historySource" aria-label="历史日线来源"><option v-for="s in dataSources.filter((s: any) => s.history)" :key="s.id" :value="s.id" :disabled="!s.available">{{ s.name }}<template v-if="!s.available">（{{ s.reason || '不可用' }}）</template></option></select></section>
      <section v-if="settingsTab === 'data'" class="settings-row"><div><strong>选股指标来源</strong><span>用于候选池估值和量价指标</span></div><select v-model="settingsDraft.screenerSource" aria-label="选股指标来源"><option v-for="s in dataSources.filter((s: any) => s.screener)" :key="s.id" :value="s.id" :disabled="!s.available">{{ s.name }}<template v-if="!s.available">（{{ s.reason || '不可用' }}）</template></option></select></section>
      <section v-if="settingsTab === 'data'" class="settings-row"><div><strong>财务数据源</strong><span>市盈率/市净率等基本面指标主数据源</span></div><select v-model="settingsDraft.fundamentalSource" aria-label="财务数据源"><option v-for="s in dataSources.filter((s: any) => s.fundamental)" :key="s.id" :value="s.id" :disabled="!s.available">{{ s.name }}<template v-if="!s.available">（{{ s.reason || '不可用' }}）</template></option></select></section>
      <template v-if="settingsTab === 'assist'">
        <section class="settings-row"><div><strong>单笔风险比例</strong><span>每次交易愿意承担的账户资金风险（用于草案建议仓位）</span></div><div class="number-input"><input v-model.number="settingsDraft.riskPerTradePct" type="number" min="0.1" max="5" step="0.1" data-testid="assist-risk" aria-label="单笔风险比例"><span>%</span></div></section>
        <section class="settings-row"><div><strong>盈亏比</strong><span>目标价 = 入场价 + 止损距离 × 盈亏比</span></div><div class="number-input"><input v-model.number="settingsDraft.rrRatio" type="number" min="1" max="10" step="0.5" data-testid="assist-rr" aria-label="盈亏比"><span>倍</span></div></section>
        <section class="settings-row"><div><strong>止损模式</strong><span>草案默认止损参照：ATR 波动止损或 MA20 均线</span></div><select v-model="settingsDraft.stopMode" data-testid="assist-stop-mode" aria-label="止损模式"><option value="atr">ATR（入场价 − 2×ATR14）</option><option value="ma20">MA20 均线</option></select></section>
        <section class="settings-row"><div><strong>单票市值上限</strong><span>建议仓位不会超过账户资金的该比例（防单票过度集中）</span></div><div class="number-input"><input v-model.number="settingsDraft.positionCapPct" type="number" min="5" max="100" step="5" data-testid="assist-cap" aria-label="单票市值上限"><span>%</span></div></section>
      </template>
      <template v-if="settingsTab === 'connection'"><section v-for="source in dataSources" :key="source.id" class="settings-row"><div><strong>{{ source.name }}</strong><span>{{ source.available ? '连接可用' : source.reason || '暂不可用' }}</span></div><div><span :class="['setting-status', { 'setting-status-muted': !source.available }]">{{ source.available ? '可用' : '不可用' }}</span><span v-if="source.realtime" class="badge badge-blue">实时</span><span v-if="source.history" class="badge badge-green">日线</span><span v-if="source.screener" class="badge badge-orange">选股</span><span v-if="source.fundamental" class="badge badge-purple">基本面</span></div></section></template>
      </div>
    </template>
  </section>
</template>
