<template>
  <section class="view-panel is-active">
    <div class="view-heading">
      <div><span class="section-kicker">STRATEGY LAB</span><h2>{{ strategyType === 'grid' ? '让区间交易有数据依据' : (strategyTypeLabel + '策略回测') }}</h2><p class="heading-note">{{ strategyType === 'grid' ? '基于复权日线测算区间并回测。结果仅用于研究，不代表未来收益。' : ('基于复权日线回测' + strategyTypeLabel + '策略。结果仅用于研究，不代表未来收益。') }}</p><p class="heading-note" v-if="strategyType === 'multi_factor'">ADX ≥ 阈值进入趋势市（唐奇安突破），否则震荡市（布林带反转）；模块连续亏损自动冻结。结果仅用于研究。</p></div>
    ...
    <div class="strategy-tabs" role="tablist" aria-label="策略类型">
      <button v-for="item in STRATEGY_TYPES" :key="item.id" type="button" role="tab" :class="['strategy-tab', { 'is-active': strategyType === item.id }]" :aria-selected="strategyType === item.id" @click="switchStrategyType(item.id)"><i :data-lucide="item.id === 'grid' ? 'grid-3x3' : item.id === 'ma_cross' ? 'trending-up' : item.id === 'dca' ? 'calendar-clock' : item.id === 'macd' ? 'activity' : item.id === 'bollinger' ? 'chart-no-axes-combined' : item.id === 'donchian' ? 'trending-up' : item.id === 'momentum' ? 'zap' : 'layers'" aria-hidden="true"></i><span>{{ item.label }}</span></button>
    </div>
