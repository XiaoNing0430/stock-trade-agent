...
网格回测使用日线数据。经典网格按每根 K 线先低后高，趋势网格按先高后低的固定顺序模拟；默认按 T+1 可卖、100 股整数倍、最低佣金、股票卖出印花税、沪市过户费和单边滑点计算，ETF 不计印花税。无成交量或无价格区间的日线不会虚构成交。参数优化采用 70% 训练期选参、30% 验证期排名。选择“每日盘后”后，策略会在每周一至周五 15:20（上海时区）自动重跑，服务重启后会从 PostgreSQL 恢复已启用任务；删除策略会同步删除其回测记录和定时任务。它用于比较参数，不构成对未来收益的承诺。
